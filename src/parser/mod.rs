...
mod ply_grammar {
    use ply::{Comment, ElementDef, Encoding, ObjInfo, PropertyDef, PropertyType, ScalarType, Version};
...
use ply::Ply;
use ply::{Encoding, Header, Payload};
...
use ply::{Addable, Comment, ElementDef, KeyMap, ObjInfo, PropertyAccess, Version};
/*
use util::LocationTracker;
use super::Parser;
use super::Line;
    ...
    fn __read_header<T: BufRead>(&self, reader: &mut T, location: &mut LocationTracker) -> Result<Header> {
        ...
        'readlines: loop {
            ...
            match line {
                ...
                Ok(Line::Format(ref t)) => match header_form_ver {
                    Some(f) if f != *t => {
        ...
        if header_form_ver.is_none() {
            return Err(io::Error::new(ErrorKind::InvalidInput, "No format line found."));
        }
...
mod tests {
    ...
    use super::Line;
    use parser::Parser;
    use ply::{Addable, DefaultElement, ElementDef, Encoding, KeyMap, PropertyDef, PropertyType, ScalarType, Version};
    ...
    #[test]
    fn format_ok() {
        assert_ok!(g::format("format ascii 1.0"), (Encoding::Ascii, Version { major: 1, minor: 0 }));
        assert_ok!(
